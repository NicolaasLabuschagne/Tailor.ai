...
export default async function GetStartedPage() {
  ...
  if (user?.businessProfile && user?.individualProfile) {
    redirect('/dashboard');
  }
    ...
    <div className="bg-gray-50 min-h-[calc(100vh-64px)] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-4xl">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 px-4 sm:px-0">
          ...
          {user?.businessProfile && (
             <div className="bg-indigo-50 overflow-hidden shadow rounded-lg border border-indigo-200 flex flex-col justify-center items-center p-6">
                <p className="text-indigo-700 font-medium">Business Profile Active</p>
                <Link href="/dashboard" className="mt-2 text-indigo-600 hover:underline">Go to Dashboard</Link>
             </div>
          )}

          {user?.individualProfile && (
             <div className="bg-indigo-50 overflow-hidden shadow rounded-lg border border-indigo-200 flex flex-col justify-center items-center p-6">
                <p className="text-indigo-700 font-medium">Individual Profile Active</p>
                <Link href="/dashboard/briefings" className="mt-2 text-indigo-600 hover:underline">Go to Briefings</Link>
             </div>
          )}
        </div>
      </div>
